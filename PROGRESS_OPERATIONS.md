...
- 用户明确授权本次连接 Windows、删除旧后羿弓 Mod 并上传 1.0.6。
- 上传前确认目标实例目录存在，Java/Minecraft 进程数为 0，唯一活动包为 `houyis-bow-1.0.5.jar`，长度 50656 字节。
- 按用户指令删除 `houyis-bow-1.0.5.jar`，并确认文件不再存在。
- 使用 `scp -O` 将 `houyis-bow-1.0.6.jar` 直接上传到目标实例 `mods` 目录。
- 上传后文件长度：50844 字节。
- 远端 SHA-256：`00B42F69FFE755FB57A895FFEC917BA0613E885384EEF9AEA5645DE46BD3B29A`，与本地一致。
- 远端活动后羿弓 JAR 数量：1；活动文件为 `houyis-bow-1.0.6.jar`。
- 远端 1.0.5 文件数量：0；Java/Minecraft 进程数仍为 0。
...
9. 每次射击是否只有一条火箭拖尾；
10. 落在方块上的后羿箭是否约 5 秒后消失；
11. 连续射击 30 秒后的卡顿是否明显改善。
